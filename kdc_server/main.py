...
# --- KDC Request Handlers ---

async def handle_as_req(data, websocket, ca_cert, db_path):
    """
    ...
    try:
        ...
        await websocket.send(json.dumps({"status": "ERROR", "message": f"Invalid request format: {e}"}))
        return
    ...
    try:
        ...
        if not ca_cert:
            await websocket.send(json.dumps({"status": "ERROR", "message": "CA certificate not loaded."}))
            return
        
        # Check validity dates
        now = datetime.datetime.now(datetime.timezone.utc)
        if now < client_cert.not_valid_before_utc or now > client_cert.not_valid_after_utc:
            await websocket.send(json.dumps({"status": "ERROR", "message": "Certificate is not currently valid (expired or not yet valid)."}))
            return

        # Ensure issuer matches your CA
        if client_cert.issuer != ca_cert.subject:
            await websocket.send(json.dumps({"status": "ERROR", "message": "Certificate issuer is not trusted."}))
            return

        ca_public_key = ca_cert.public_key()
        if not isinstance(ca_public_key, rsa.RSAPublicKey):
            await websocket.send(json.dumps({"status": "ERROR", "message": "CA certificate must use RSA public key."}))
            return
        
        if client_cert.signature_hash_algorithm is None:
            await websocket.send(json.dumps({"status": "ERROR", "message": "Unsupported signature algorithm."}))
            return
        ...
        try:
            ...
            await websocket.send(json.dumps({"status": "ERROR", "message": "Certificate signature is invalid."}))
            return

        # Simplified check: Is the certificate's issuer our CA?
        if client_cert.issuer != ca_cert.subject:
            await websocket.send(json.dumps({"status": "ERROR", "message": "Certificate issuer is not trusted."}))
            return
        ...
        await websocket.send(json.dumps({"status": "ERROR", "message": f"Certificate trust check failed: {e}"}))
        return
    ...
    try:
        ...
        if not principal_row:
            await websocket.send(json.dumps({"status": "ERROR", "message": "Unknown or untrusted device for presented certificate."}))
            return
        ...
        if not tgs_row:
            await websocket.send(json.dumps({"status": "ERROR", "message": "KDC is misconfigured: TGS principal not found."}))
            return
        
        tgs_secret_key = base64.b64decode(tgs_row['secret_key_b64'])

    except Exception as e:
        await websocket.send(json.dumps({"status": "ERROR", "message": f"Database lookup failed: {e}"}))
        return
    ...
    try:
        ...
        await websocket.send(json.dumps({"status": "ERROR", "message": "Invalid signature. Proof of possession failed."}))
        return
    except Exception as e:
        await websocket.send(json.dumps({"status": "ERROR", "message": f"Signature verification error: {e}"}))
        return
    ...
    try:
        ...
        if data["principal"] != principal_name and data["principal"] != cert_principal:
            await websocket.send(json.dumps({"status": "ERROR", "message": "Principal name does not match certificate or registered principal."}))
            return
        ...
        try:
            ...
            await websocket.send(json.dumps({"status": "ERROR", "message": "Invalid timestamp format."}))
            return

        if abs((datetime.datetime.now(datetime.timezone.utc) - ts).total_seconds()) > TIMESTAMP_WINDOW_SECONDS:
            await websocket.send(json.dumps({"status": "ERROR", "message": "Timestamp is outside the allowed window (possible replay attack)."}))
            return

    except Exception as e:
        await websocket.send(json.dumps({"status": "ERROR", "message": f"Post-verification checks failed: {e}"}))
        return
    ...
    try:
        ...
        await websocket.send(json.dumps({"status": "ERROR", "message": f"Error generating TGT: {e}"}))
        return
...
# --- TGS Request Handler ---
async def handle_tgs_req(data, websocket, ca_cert, db_path):
    """
    ...
    try:
        ...
        conn.close()
        if not tgs_row:
            await websocket.send(json.dumps({"status": "ERROR", "message": "KDC misconfigured: TGS principal not found."}))
            return
        tgs_secret_key = base64.b64decode(tgs_row['secret_key_b64'])
    except Exception as e:
        await websocket.send(json.dumps({"status": "ERROR", "message": f"Failed to retrieve TGS key: {e}"}))
        return
    ...
    try:
        ...
        if datetime.datetime.now(datetime.timezone.utc) > expiry_time:
            await websocket.send(json.dumps({"status": "ERROR", "message": "TGT has expired."}))
            return
        ...
        await websocket.send(json.dumps({"status": "ERROR", "message": f"Failed to decrypt TGT: {e}"}))
        return
    ...
    try:
        ...
        if abs((datetime.datetime.now(datetime.timezone.utc) - auth_ts).total_seconds()) >TIMESTAMP_WINDOW_SECONDS:
            await websocket.send(json.dumps({"status": "ERROR", "message": "Authenticator timestamp is outside the allowed window (possible replay attack)."}))
            return
        
        # Check if principal in TGT matches principal in Authenticator
        if auth_data['principal'] != principal_from_tgt:
            await websocket.send(json.dumps({"status": "ERROR", "message": "Principal in Authenticator does not match TGT principal."}))
            return
        ...
        await websocket.send(json.dumps({"status": "ERROR", "message": f"Failed to decrypt Authenticator: {e}"}))
        return
    ...
    try:
        ...
        if not service_row:
            await websocket.send(json.dumps({"status": "ERROR", "message": f"Unknown service principal: {service_principal}"}))
            return
        service_secret_key = base64.b64decode(service_row['secret_key_b64'])

    except Exception as e:
        await websocket.send(json.dumps({"status": "ERROR", "message": f"Failed to retrieve service key: {e}"}))
        return
    ...
    try:
        ...
        await websocket.send(json.dumps({"status": "ERROR", "message": f"Error generating Service Ticket: {e}"}))
        return
...
async def handle_list_services(data, websocket, db_path):
    """
    Handles the request to list all available services.
    ...
    """
    print("Handling LIST_SERVICES...")

    try:
        conn = get_db_conn(db_path)
        cursor = conn.cursor()
        cursor.execute("SELECT principal_name, service_name, service_url, description FROM services WHERE principal_name != ?", (f"tgs@{REALM}",))
        services = [{"principal_name": row['principal_name'], "name": row['service_name'], "url": row['service_url'], "description": row['description']} for row in cursor.fetchall()]
        conn.close()
        ...
        await websocket.send(json.dumps({"status": "ERROR", "message": f"Failed to list services: {e}"}))

# --- Main Functions ---

async def start_kdc(role, host='0.0.0.0', port=8888):
    ...
    async def handle_connection(websocket):
        ...
        try:
            ...
            # Load CA cert only for actual requests
            ca_cert = load_ca_cert()
            if not ca_cert:
                await websocket.send(json.dumps({"status": "ERROR", "message": "KDC is not properly configured (Missing CA)"}))
                return
            
            if data.get('type', '').strip().upper() == 'AS_REQ':
                await handle_as_req(data, websocket, ca_cert, db_path)
            elif data.get('type', '').strip().upper() == 'TGS_REQ':
                await handle_tgs_req(data, websocket, ca_cert, db_path)
            elif data.get('type', '').strip().upper() == 'LIST_SERVICES':
                await handle_list_services(data, websocket, db_path)
            else:
    ...
    async with websockets.serve(handle_connection, host, port):
        print(f"KDC WebSocket service listening on {host}:{port}")
...
if __name__ == "__main__":
    ...
    asyncio.run(start_kdc(args.role, host='0.0.0.0', port=port))
